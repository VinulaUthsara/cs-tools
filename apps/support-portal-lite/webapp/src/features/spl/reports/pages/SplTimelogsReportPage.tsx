...
// Ported from one-wso2's features/spl/reports/pages/SplTimelogsReportPage.tsx
// (itself ported from the source app's pages/TimelogsReport.tsx). Route:
// projects/:projectId/timelogs-report.
...
export default function SplTimelogsReportPage() {
  ...
  // Local replacement for one-wso2's app-wide NotificationsContext, which
  // this standalone app doesn't have — a single inline Alert is enough for
  // the one message this page ever shows.
  ...
  const handleRowClick = (rowData: DataStruct) => {
    if (rowData?.caseNumber !== "") {
      window.open(`/cases/${rowData.caseNumber}`, "_blank");
    } else {
