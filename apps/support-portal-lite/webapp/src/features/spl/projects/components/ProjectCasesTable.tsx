...
// Case links point at /cases/:caseId (this app's own route — no "support-"
// prefix, unlike one-wso2's /csm/support-cases, which only existed there to
// avoid colliding with a sibling CSM route this app doesn't have).
...
export default function ProjectCasesTable({ id, isTypeCloud }: { id: string; isTypeCloud: boolean }) {
  ...
  const handleCaseTableRowClick = (rowData: CaseDetails) => {
    window.open(`/cases/${rowData.number}`, "_blank");
  };
