...
// Unlike one-wso2's version, this app has no notifications context to
// report a missing sysId through — the report buttons are simply hidden
// until data.sysId is available, rather than opening and then toasting an
// error.
...
function ProjectHeading({ data }: { data: ProjectDetails }) {
  ...
  const openReport = (path: string) => {
    ...
    window.open(`/projects/${data.number}/${path}/${data.sysId}`);
  };

  const openTimelogs = () => {
    window.open(`/projects/${data.number}/timelogs-report`);
  };
