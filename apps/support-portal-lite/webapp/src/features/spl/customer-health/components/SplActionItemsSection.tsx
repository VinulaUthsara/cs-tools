...
// Ported from one-wso2's SplActionItemsSection.tsx, restyled onto
// @wso2/oxygen-ui. useNotifications() there came from a one-wso2-only
// context — swapped for this domain's own useHealthToast (see
// ../hooks/healthToastContext.ts for why oxygen-ui's own useNotifications
...
import { useHealthToast } from "../hooks/healthToastContext";
...
function AddActionItemDialog({ open, onClose, accountId, projectsWithOpenRisks, onCreated, defaultProjectSysId }: AddActionItemDialogProps) {
  ...
  const { showSuccess, showError } = useHealthToast();
...
function InlineCommentThread({ itemId, onCommentPosted }: InlineCommentThreadProps) {
  ...
  const httpRequest = useSplHttpRequest();
  const { showError } = useHealthToast();
