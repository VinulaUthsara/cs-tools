...
// Ported from one-wso2's SplCustomerHealthDetailPage.tsx, restyled onto
// @wso2/oxygen-ui.
...
import { HealthToastProvider } from "../hooks/HealthToastProvider";
...
function CustomerHealthDetailContent() {
  ...
  const navigateTo = async (targetAbsoluteIdx: number) => {
    ...
    if (targetAbsoluteIdx >= localRangeStart && targetAbsoluteIdx <= localRangeEnd) {
      ...
      navigate(`/customer-health/account/${accountList[localIdx].accountSysId}`, {
    ...
    if (account) {
      navigate(`/customer-health/account/${account.accountSysId}`, {
  ...
  if (!accountId) {
    return (
      <Box>
        <Link to="/customer-health" style={{ textDecoration: "none" }}>
