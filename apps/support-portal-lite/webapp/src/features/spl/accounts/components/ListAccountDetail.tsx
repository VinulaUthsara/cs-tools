...
// Ported from one-wso2's features/spl/accounts/components/ListAccountDetail.tsx
...
function BasicInfoTable({ data }: { data: AccountDetails }) {
  ...
  const handleTeamScheduleClick = () => {
    if (integrationCSTeamSysId !== "") {
      // No /csm prefix here — this app's own team-schedule route lives at
      // /team-schedule, not /csm/team-schedule (that prefix only existed in
      // one-wso2 to sit alongside its sibling CSM route).
      window.open(`/team-schedule/${integrationCSTeamSysId}`, "_blank");
    } else {
