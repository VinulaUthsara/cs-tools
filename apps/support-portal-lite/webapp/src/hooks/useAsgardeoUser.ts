...
const SIGNED_OUT: AsgardeoUser = { ready: false, initials: "" };

export function useAsgardeoUser(): AsgardeoUser {
  const { isSignedIn, getDecodedIdToken } = useAsgardeo();
  const [user, setUser] = useState<AsgardeoUser>(SIGNED_OUT);

  useEffect(() => {
    // No setState here for the signed-out case — this effect's job is
    // subscribing to the async decode below, not synchronizing `isSignedIn`
    // itself (the render-time check after this hook's effect already
    // returns SIGNED_OUT directly, without going through state at all).
